...
with tab_editor:
    ...
    with col22:
        ...
        with st.popover(USUAL_ICONS.EXPANDER.value):
            ...
            DATAFRAME = pd.DataFrame(SQL)
            print(DATAFRAME)
            st.selectbox("DEVICE TYPE", options=DATAFRAME['DEVICE_TYPE'].unique().tolist(), index=None)
            st.selectbox("MANUFACTURER", options=sorted(DATAFRAME['MANUFACTURER'].unique().tolist()), index=None)
            st.selectbox("MODEL", options=DATAFRAME['MODEL'].unique().tolist(), index=None)
